...
class JSPRAW extends Base {
  constructor(opts) {
    ...
    this
      .encoders
      .map((_) => {
        this.parseEncoder(`./jspraw/encoder/${_}`);
      });
    this
      .decoders
      .map((_) => {
        this.parseDecoder(`./jspraw/decoder/${_}`);
      });
  }
  ...
  complete(data, force_default = false) {
    ...
    data['_'] = this.replaceClassStringVar(data['_'], 'antswordargdecoder', formatter['base64'](asencCode));
    // 使用编码器进行处理并返回
              ...
              (args[arg].match(/#{([\w\:]+)}/g) || []).map(
                ...
                (tag) => {
                  ...
                  if ((tagArr.length > 0) && (func = formatter[tagArr[0]])) {
                    ...
                    data['_'] = this.replaceClassStringVar(data['_'], `antswordarg${tagArr[1]}`, retStr);
                  } else {
                    // 否则替换直接返回字符串
                    retStr = argv[tagStr] || '';
                    data['_'] = this.replaceClassStringVar(data['_'], `antswordarg${tagStr}`, retStr);
                  }
                });
            }
            // 发送HTTP请求
